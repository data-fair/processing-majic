...
module.exports = async (proccesingConfig, dir = 'data', log) => {
  const dirName = __dirname.split(path.sep).slice(0, -1).join(path.sep)
  await log.step('Traitement des données')
  await fs.createFileSync(path.join(dirName + '/' + dir, 'Parcelles des personnes morales.csv'))
  ...
  for (const depFile of depFiles) {
    ...
    const data = iconv.decode(await fs.readFileSync(path.join(dirName, dir, depFile)), 'iso-8859-1')
